...
/* special length/indicator valu            es */
...
#define DSQL_DIAG_FUNC_CODE_CREATE_DIRECTORY            91  /* create directory语句类型 */
...
TEXT以字符为单位
[LLOB-des-5]原有接口不做修改
32位的情况下ulength仅有4字节，无法支持超长的数据
此时若输出参数的值超过slength的数据范围时报错
*******************************/
DllExport
DPIRETURN
dpi_lob_get_length(
...
TEXT以字符为单位
[LLOB-des-5]原有接口不做修改
32位的情况下ulength仅有4字节，无法支持start_pos超出范围的情况
*******************************/
DllExport
DPIRETURN
dpi_lob_read(
    dhloblctr       dpi_loblctr,
    ulength         start_pos,      //1 based:blob bytes clob chars
    sdint2          ctype,
    slength         data_to_read,   //0:default, blob bytes clob chars
    dpointer        val_buf,
    slength         buf_len,
    slength*        data_get        //blob:bytes_get clob:chars get
);
