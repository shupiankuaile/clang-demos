...
int WRPG_init() {
    ...
    gWindow = SDL_CreateWindow( "WRPG test21 frame rate limit", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 
    ...
    //关闭垂直同步，我们需要手动限制帧率，否则帧率会非常高，因为程序的运行是非常快的
...
void WRPG_mainLoop() {
    ...
    while(!stop) {
        ...
        uint32_t costPerFrame = SDL_GetTicks();
        ...
        uint32_t elapseTime = SDL_GetTicks() - startTime;
        float avgFPS = 0.f;
        if(elapseTime == 0 ) {
            avgFPS = 0.f;
        } else {
        ...
        costPerFrame = SDL_GetTicks() - costPerFrame;
        if(costPerFrame < SCREEN_TICKS_PER_FRAME) {
            //等待剩余时间
            SDL_Delay( SCREEN_TICKS_PER_FRAME - costPerFrame );
        }
