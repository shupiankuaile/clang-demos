...
void WRPG_initActor(WRPG_actor* actor, SDL_Renderer* renderer) {
    ...
    actor->mCollider.x = DOT_WIDTH/2;
    actor->mCollider.y = DOT_HEIGHT/2;
    actor->mCollider.r = DOT_WIDTH/2;
...
void WRPG_move(WRPG_actor* actor, SDL_Rect wallCollider, WRPG_circle circleCollider) {

    //向左或向右移动点
    actor->mPosX += actor->mVelX;
    actor->mCollider.x = actor->mPosX + DOT_WIDTH/2;

    //如果点向左或向右走得太远的话
    if( ( actor->mPosX < 0 ) 
            || ( actor->mPosX + DOT_WIDTH > SCREEN_WIDTH )
            || (checkCircleRectCollision(actor->mCollider, wallCollider)) 
            || (checkCircleCollision(actor->mCollider, circleCollider)) ) {
        //退回
        actor->mPosX -= actor->mVelX;
        actor->mCollider.x = actor->mPosX + DOT_WIDTH/2;
    }
    ...
    actor->mCollider.y = actor->mPosY + DOT_HEIGHT/2;

    //如果点向上或向下走得太远
    if( ( actor->mPosY < 0 ) 
            || ( actor->mPosY + DOT_HEIGHT > SCREEN_HEIGHT )
            || (checkCircleRectCollision(actor->mCollider, wallCollider)) 
            || (checkCircleCollision(actor->mCollider, circleCollider)) ) {
        //退回
        actor->mPosY -= actor->mVelY;
