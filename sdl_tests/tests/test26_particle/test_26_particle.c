...
#define ACTOR_PARTICLES 20
WRPG_particle* gParticles[ ACTOR_PARTICLES ];
...
void WRPG_mainLoop() {
    ...
    for(int i=0; i<ACTOR_PARTICLES; i++) {
        gParticles[i] = WRPG_initParticle(gRenderer, actor.mPosX, actor.mPosY);
    }
    ...
    while(!stop) {
        ...
        for(int i=0; i<ACTOR_PARTICLES; i++) {
            if(WRPG_isDead(gParticles[i])) {
                WRPG_freeParticle(gParticles[i]);
                gParticles[i] = WRPG_initParticle(gRenderer, actor.mPosX, actor.mPosY);
            }

            WRPG_renderParticle(gParticles[i]);
        }
